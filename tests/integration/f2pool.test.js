...
test('Integration: mock rejects invalid currency on balance', async (t) => {
  const res = await makeRequest(mockServerPort, '/v2/assets/balance', {
    body: {
      currency: 'not-a-real-currency',
      mining_user_name: 'testuser'
    },
    headers: { 'F2P-API-SECRET': 'secret-key' }
  })
  t.is(res.statusCode, 200)
  t.is(res.body.code, 5001)
})

test('Integration: mock server applies response delay', async (t) => {
  const testPort = await new Promise((resolve) => {
    const server = http.createServer()
    server.listen(0, '127.0.0.1', () => {
      const port = server.address().port
  ...
  await new Promise((resolve) => setTimeout(resolve, 100))
  const t0 = Date.now()
  await makeRequest(testPort, '/v2/assets/balance', {
    body: { currency: 'bitcoin', mining_user_name: 'testuser' },
    headers: { 'F2P-API-SECRET': 'secret-key' }
  })
  t.ok(Date.now() - t0 >= 60, 'delayed onSend should add latency')
  await delayed.stop()
})
